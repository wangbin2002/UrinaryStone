...
class Config:
    ...
    save_dir = "Resnet18_experiment_results"
...
# 绘制并保存ROC曲线
def plot_roc_curve(y_true, y_prob, classes, save_path, timestamp, dpi=1200):
    # 计算每个类别的ROC曲线
    fpr = dict()
    tpr = dict()
    roc_auc = dict()
    ...
    for i, color in zip(range(n_classes), colors):
        plt.plot(fpr[i], tpr[i], color=color, lw=2,
                 label=f'ROC curve of class {classes[i]} (AUC = {roc_auc[i]:.2f})',fontsize=16)
    
    plt.plot(fpr["macro"], tpr["macro"],
             label=f'Macro-average ROC curve (AUC = {roc_auc["macro"]:.2f})',
             color='navy', linestyle=':', linewidth=4,fontsize=16)
...
# 绘制并保存PR曲线
def plot_pr_curve(y_true, y_prob, classes, save_path, timestamp, dpi=1200):
    # 计算每个类别的PR曲线
    precision = dict()
    recall = dict()
    ...
    for i, color in zip(range(n_classes), colors):
        plt.plot(recall[i], precision[i], color=color, lw=2,
                 label=f'PR curve of class {classes[i]} (AP = {average_precision[i]:.2f})',fontsize=16)
...
# 绘制并保存混淆矩阵
def plot_confusion_matrix(y_true, y_pred, classes, save_path, timestamp, dpi=1200, normalize=True):
    # 计算混淆矩阵
    cm = confusion_matrix(y_true, y_pred, labels=range(len(classes)),fontsize=16)
...
# 绘制DCA曲线
def plot_dca_curve(y_true, y_prob, classes, save_path, timestamp, dpi=1200):
    n_classes = len(classes)
...
# 绘制CIC曲线
def plot_cic_curve(y_true, y_pred, y_prob, classes, save_path, timestamp, dpi=1200):
    n_classes = len(classes)
    n_samples = len(y_true)
